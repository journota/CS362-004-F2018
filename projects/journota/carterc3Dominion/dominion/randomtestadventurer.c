...
** Date: 11/14/18
** Description: Random test 3 for dominion.c tests adventurer implementation
*********************************************************************/
...
int main (){
    ...
    // randomization of kingdom cards does not matter as they do not affect the outcome of the affects of adventurer
    int kCards[10] = {feast, adventurer, cutpurse, minion, smithy, tribute, sea_hag, great_hall, baron};

    // randomization of choice does not matter as these do not affect adventurer
    int choice1 = 0;
    int choice2 = 0;
    int choice3 = 0;
    ...
    for (d = 0; d < TEST_COUNT; d++){
        ...
        for (m = 0; m < players; m++){
            ...
            for (n = 0; n < game.handCount[m]; n++){
                ...
                // if 0th position card, set adventurer, if 0-7, set from base cards,
                // otherwise set from in-game kingdom cards
                if (n == 0){
                    game.hand[m][n] = adventurer;
                } else if (tempCard < 7){
        ...
        for(i = p, j = 0; j < players; i++, j++){
            ...
            if(i == p){
                if(TEST_PRINT){
                    ...
                    (game.deckCount[i] + (game.discardCount[i] - test.discardCount[i]) - cardDelta),
                    (game.playedCardCount + discard), (game.discardCount[i] + (game.deckCount[i] - test.deckCount[i]) - cardDelta),
                ...
                if((game.deckCount[i] + (game.discardCount[i] - test.discardCount[i]) - cardDelta) != test.deckCount[i]){ failFlag = 1; }
                if((game.playedCardCount + discard) != test.playedCardCount){ failFlag = 1; }
                if((game.discardCount[i] + (game.deckCount[i] - test.deckCount[i]) - cardDelta) != test.discardCount[i]){ failFlag = 1; }
            }
            else{
                if(TEST_PRINT){
