...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      playAdventurer(state, currentPlayer, temphand, z);
...
/*****************************************************************************/
/*                    Refactored functions added here                        */
/*****************************************************************************/
    ...
    for (i = 0; i < state->numPlayers; i++) {
	    if (i != currentPlayer) {
            for (j = 0; j < state->handCount[i]; j++) {
                ...
                if (j == state->handCount[j]) { /*BUG INTRODUCED HERE THAT CHANGES i TO j | This should change the hand counter to look at the incorrect array parameters*/
                    for (k = 0; k < state->handCount[i]; k++) {
                        if (DEBUG) {
                            printf("Player %d reveals card number %d\n", i, state->hand[i][k]);
                        }
...
int playBaron(struct gameState *state, int currentPlayer, int choice1){
    ...
    if (choice1 > 0){//Boolean true or going to discard an estate
        ...
        while(card_not_discarded){
	        if (state->hand[currentPlayer][p] == estate){//Found an estate card! 
	            state->coins = 4;//Add 4 coins to the amount of coins /*BUG INTRODUCED HERE THAT CHANGES += TO = | This should reset the player coin count to 4 instead of adding 4*/
	            state->discard[currentPlayer][state->discardCount[currentPlayer]] = state->hand[currentPlayer][p];
	            state->discardCount[currentPlayer]++;
	        for (;p < state->handCount[currentPlayer]; p++){
	            state->hand[currentPlayer][p] = state->hand[currentPlayer][p+1];
...
int playAdventurer(struct gameState *state, int currentPlayer, int *temphand, int z){
     ...
     // draw from player dect one card at a time until 2 treasure cards are revealed
     // once two treasure cards are revealed, add them to the player hand and discard all other cards
    ...
    while(drawntreasure<2){
      ...
      // if the card is a treasure, keep it
      if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
	      drawntreasure++;
    ...
    while(z-1>0){ /*BUG INTRODUCED HERE THAT CHANGES >= TO > | This should leave a card in the player's hand that should have been discarded*/
